        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('model'):
                with tf.variable_scope('dense_layers'):
                    self.fcl1 = tf.layers.dense(inputs = self.input_tensor,
                                            units = 256,
                                            activation = tf.tanh,
                    ...
                    self.fcl2 = tf.layers.dense(inputs = self.fcl1,
                                            ...
                                            bias_initializer = tf.constant_initializer(),
                                            name = 'fcl2')
                    self.fcl3 = tf.layers.dense(inputs = self.fcl2,
                                            units = action_n * value_n,
                                            activation = None,
                                            kernel_initializer = tf.contrib.layers.xavier_initializer(),
                                            bias_initializer = tf.constant_initializer(),
                                            name = 'linear')
                self.output_tensor = tf.reshape(self.fcl3, [batch_size, action_n, value_n], name='output')
            ...
            if batch_size_large is not None:
                with tf.name_scope('large_eval'):
                    ...
                    with tf.variable_scope('model', reuse=True):
                        with tf.variable_scope('dense_layers'):
                            self.fcl1_large = tf.layers.dense(inputs = self.input_tensor_large,
                                                    units = 256,
            ...
            if learning_rate is not None:
                ...
                with tf.name_scope('endpoints'):
                    ...
                    tf.summary.histogram('fcl2', self.fcl2)
                    tf.summary.histogram('fcl3', self.fcl3)
